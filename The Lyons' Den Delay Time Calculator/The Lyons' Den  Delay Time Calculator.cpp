...
int main()
{
    ...
    } while (RepeatProgram[0]!='q' && RepeatProgram[0]!='Q');
}
...
void CalculateAndDisplayDelayTimes(float Temp)
{
    ...
    cout << fixed << setprecision(0);//rounds back to whole after program loops - else it rounds to 100ths on second+ loop
    ...
    // 1/1 notes
    usleep(USLEEP); cout << " 1/1  = " << setw(MS_SETW) << (MILLISECONDS/Temp)*4         << " " << MillisecondName << "\n\n";
    
    // 1/2 notes
    usleep(USLEEP); cout << " 1/2. = " << setw(MS_SETW) << (MILLISECONDS/Temp)*(3.0/1)   << " " << MillisecondName <<   "\n";
    usleep(USLEEP); cout << " 1/2  = " << setw(MS_SETW) << (MILLISECONDS/Temp)*2         << " " << MillisecondName <<   "\n";
    usleep(USLEEP); cout << " 1/2T = " << setw(MS_SETW) << (MILLISECONDS/Temp)*(4.0/3)   << " " << MillisecondName << "\n\n";
    
    // 1/4 notes
    usleep(USLEEP); cout << " 1/4. = " << setw(MS_SETW) << (MILLISECONDS/Temp)*(3.0/2)   << " " << MillisecondName <<   "\n";
    usleep(USLEEP); cout << " 1/4  = " << setw(MS_SETW) << (MILLISECONDS/Temp)           << " " << MillisecondName <<   "\n";;//starting point
    usleep(USLEEP); cout << " 1/4T = " << setw(MS_SETW) << (MILLISECONDS/Temp)*(2.0/3)   << " " << MillisecondName << "\n\n";
    
    // 1/8 notes
    usleep(USLEEP); cout << " 1/8. = " << setw(MS_SETW) << (MILLISECONDS/Temp)*(3.0/4)   << " " << MillisecondName <<   "\n";
    usleep(USLEEP); cout << " 1/8  = " << setw(MS_SETW) << (MILLISECONDS/Temp)/2         << " " << MillisecondName <<   "\n";
    usleep(USLEEP); cout << " 1/8T = " << setw(MS_SETW) << (MILLISECONDS/Temp)*(1.0/3)   << " " << MillisecondName << "\n\n";
    
    // 1/16 notes
    usleep(USLEEP); cout << "1/16. = " << setw(MS_SETW) << (MILLISECONDS/Temp)*(3.0/8)   << " " << MillisecondName <<   "\n";
    usleep(USLEEP); cout << "1/16  = " << setw(MS_SETW) << (MILLISECONDS/Temp)/4         << " " << MillisecondName <<   "\n";
    usleep(USLEEP); cout << "1/16T = " << setw(MS_SETW) << (MILLISECONDS/Temp)*(0.5/3)   << " " << MillisecondName << "\n\n";
    
    // 1/32 notes
    usleep(USLEEP); cout << "1/32. = " << setw(MS_SETW) << (MILLISECONDS/Temp)*(3.0/16)  << " " << MillisecondName <<   "\n";
    usleep(USLEEP); cout << "1/32  = " << setw(MS_SETW) << (MILLISECONDS/Temp)/8         << " " << MillisecondName <<   "\n";
    usleep(USLEEP); cout << "1/32T = " << setw(MS_SETW) << (MILLISECONDS/Temp)*(0.25/3)  << " " << MillisecondName << "\n\n";
    
    // 1/64 notes
    usleep(USLEEP); cout << "1/64. = " << setw(MS_SETW) << (MILLISECONDS/Temp)*(3.0/32)  << " " << MillisecondName <<   "\n";
    usleep(USLEEP); cout << "1/64  = " << setw(MS_SETW) << (MILLISECONDS/Temp)/16        << " " << MillisecondName <<   "\n";
    usleep(USLEEP); cout << "1/64T = " << setw(MS_SETW) << (MILLISECONDS/Temp)*(0.125/3) << " " << MillisecondName << "\n\n";
